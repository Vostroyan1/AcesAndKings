...
public class AceFoundationController extends MouseAdapter {
	...
	public void mouseReleased(MouseEvent me) {
		...
		if (fromWidget.getName().contains("Reserve")){ //if card is from Reserve column
			Pile acefoundation = (Pile) src.getModelElement();
			Column reserve = (Column) fromWidget.getModelElement();
			
			CardView cardView = (CardView) draggingWidget;
			Card theCard = (Card) cardView.getModelElement();
			
			Move move = new ReserveToAceFoundationMove(reserve, theCard, acefoundation);
			...
			if (fromWidget.getName().contains("Waste")){ //if card is from Waste pile
				...
				//System.out.println(src.getName()); //KingFoundationNum
				//System.out.println(fromWidget.getName()); //SourceNameNum
			
				CardView cardView = (CardView) draggingWidget;
				Card theCard = (Card) cardView.getModelElement();
				
				Move move = new WasteToAceFoundationMove(waste, theCard, acefoundation);
				if (move.doMove(theGame)) {
					theGame.pushMove (move);     // Successful Move has been Move
				} else {
				fromWidget.returnWidget (draggingWidget);
			...
			if (fromWidget.getName().contains("Foundation")){ //if card is from a foundation pile
				...
				Move move = new FoundationToAceFoundationMove(foundation, theCard, kingfoundation);
				if (move.doMove(theGame)) {
					theGame.pushMove (move);     // Successful Move has been Move
				} else {
				fromWidget.returnWidget (draggingWidget);
